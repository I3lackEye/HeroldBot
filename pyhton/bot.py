...
STATS_FILE_PATH = os.environ["STATS_DATABASE_PATH"]
        ...
        try:
            with open(FILE_PATH, "r", encoding="utf-8") as file:
                ...
                if not isinstance(data, dict):
                    print("⚠ Error: Stats file format is incorrect! Creating new stats file.")
                    bot_logger.info("⚠ Error: Stats file format is incorrect! Creating new stats file.")
                    return {"teams": {}, "solo": [], "punkte": {}}
                return data
        except json.JSONDecodeError:
            print("⚠ Fehler: Datei ist beschädigt! Erstelle eine leere Datei.")
            bot_logger.info("Fehler: Datei ist beschädigt! Erstelle eine leere Datei.")
            return {"teams": {}, "solo": [], "punkte": {}}
    return {"teams": {}, "solo": [], "punkte": {}}  # Falls Datei nicht existiert

def load_stats():
    """
    Loads tournament statistics from file.
    The expected format is a JSON object with a "players" key,
    where each key is a player's name and its value is a list of tournaments won.
    
    :return: dict containing tournament stats, e.g. {"players": {"Player1": ["Tournament A", ...]}}
    """
    if os.path.exists(STATS_FILE_PATH):
        try:
            ...
            print("⚠ Error: Stats file is corrupted! Creating a new empty stats file.")
            bot_logger.info("Error: Stats file is corrupted! Creating a new empty stats file.")
            return {"players": {}}
    return {"players": {}}

def save_stats(stats_data):
    """
    Saves tournament statistics to file.
    :param stats_data: dict containing tournament stats to save.
    """
    with open(STATS_FILE_PATH, "w", encoding="utf-8") as file:
        json.dump(stats_data, file, indent=4, ensure_ascii=False)

def update_player_stats(player_name: str, tournament_name: str, stats_data: dict):
    """
    Updates the statistics for a given player by adding a new tournament win.
    If the player is not already in the stats, they will be added.
    :param player_name: Name of the player.
    :param tournament_name: Name of the tournament that the player has won.
    :param stats_data: Current stats dictionary.
    """
    if "players" not in stats_data:
        stats_data["players"] = {}
    if player_name not in stats_data["players"]:
        stats_data["players"][player_name] = []
    
    # Add tournament_name if not already present
    if tournament_name not in stats_data["players"][player_name]:
        stats_data["players"][player_name].append(tournament_name)
...
@tree.command(name="new_game", description="Start a poll to add a new game for future tournaments.")
async def new_game(interaction: discord.Interaction, game_name: str, duration: int):
    # Check if the command is used in the correct channel (optional)
    if interaction.channel_id != LIMITED_CHANNEL_ID_1:
        await interaction.response.send_message("🚫 This command can only be used in a specific channel!", ephemeral=True)
        return

    # Acknowledge the command immediately (ephemeral to the command issuer)
    await interaction.response.send_message("✅ New game poll started!", ephemeral=True)
    
    # Send the poll message in the channel
    poll_message = await interaction.channel.send(
        f"**New Game Suggestion:** **{game_name}**\n"
        "React with 👍 to approve or 👎 to reject this suggestion."
    )
    
    # Add initial reactions (the bot's own reactions to prompt voting)
    await poll_message.add_reaction("👍")
    await poll_message.add_reaction("👎")
    
    # Define the duration for the poll (in seconds)
    poll_duration = duration  # You can adjust this duration as needed
    
    async def finalize_poll():
        # Wait for the poll duration before counting reactions
        await asyncio.sleep(poll_duration)
        # Refresh the message to get updated reaction counts
        updated_message = await interaction.channel.fetch_message(poll_message.id)
        
        thumbs_up = 0
        thumbs_down = 0
        # Subtract the bot's own reaction from the count (if applicable)
        ...
        # Determine if the game suggestion is approved
        if thumbs_up > thumbs_down:
            # Initialize the games list if it doesn't exist
            if "games" not in anmeldungen:
                anmeldungen["games"] = []
            # Add the game if it's not already in the list
            if game_name not in anmeldungen["games"]:
                anmeldungen["games"].append(game_name)
                save_anmeldungen()
            await interaction.channel.send(f"🎮 Game **{game_name}** has been added to the future tournaments list!")
        else:
            await interaction.channel.send(f"❌ Game **{game_name}** was not approved and will not be added.")
    
    # Run the poll finalization as a background task
    asyncio.create_task(finalize_poll())
